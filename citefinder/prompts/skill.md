...
## Keeping this skill in sync with the installed version

This file ships **inside** the citefinder package and is materialized by
`citefinder install`. If it carries a `<!-- generated by citefinder ... -->`
comment under the frontmatter, it is a generated copy — edit
`citefinder/prompts/skill.md` in the citefinder repo, not this file.

After upgrading citefinder, confirm the copy still matches the installed
version, and re-materialize it if not:

```bash
citefinder install --check   # ok | drifted | missing (exits 1 unless ok)
citefinder install --force   # re-materialize at the installed version
```

Add `--local` to both when the skill is vendored in a repo's `.claude/`
rather than in `~/.claude/`. A `drifted` result means the documented commands
may not match the installed CLI — re-materialize before trusting the examples
below.
