...
def install(
    local: bool = typer.Option(
        False,
        "--local",
        help="Install into the current repo's .claude/ instead of ~/.claude/.",
    ),
    ...
    if check:
        ...
        # auto-resolves whichever copy is installed and recovers its stamped
        # mode, so it needs no flags to match how the skill was installed.
        if local:
            status = install_mod.check_mode(root, version, mode)
            where: Path | None = install_mod.skill_path(root, mode)
        else:
            status = install_mod.check(root, version)
            found = install_mod.resolve_installed(root)
            where = found[0] if found else None
            mode = install_mod.resolve_mode(root)
    ...
    if path.exists() and not install_mod.is_generated(path) and not force:
        typer.echo(
