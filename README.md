...
the package**, under `citefinder/prompts/skills/use-citefinder/`, laid out per
the [Agent Skills specification](https://agentskills.io/specification): a
`SKILL.md` and a `references/` directory. It is never copied out. The
...
The packaging, stub rendering, and drift check come from
[pkgskills](https://pypi.org/project/pkgskills/); `citefinder/host.py` declares
what citefinder ships.
...
on, which have to be readable off disk) plus a pointer to `citefinder skill use-citefinder`:
...
```
<!-- generated by citefinder 0.11.2a0 via pkgskills 0.5.1 (mode=global); do not edit. Regenerate with: citefinder install --force -->
```

The stub changes rarely, but it is still checkable — `--check` compares it
against what the *currently installed* citefinder would render (ignoring both
stamped versions, so a routine upgrade of citefinder or pkgskills alone never
reads as drift), prints one row per stub with a reason on anything not `ok`,
and exits non-zero unless every row is `ok`:
...
safe. A stub written by a release before citefinder adopted pkgskills reports
`foreign`; `install --force` replaces it. To change the skill's content, edit
`citefinder/prompts/skills/use-citefinder/` and release — there is no
`.claude/` copy to re-sync.
