...
citefinder install                                          # into ~/.claude/
...
citefinder ships the `use-citefinder` [Claude Code](https://claude.ai/code)
skill inside the package, at `citefinder/prompts/skill.md`. `citefinder
install` materializes it into a `.claude/skills/` tree:
...
The installed file is the bundled body plus one stamp comment after the
frontmatter:
...
The stamp is what makes the copy checkable: it records the version the skill
was rendered from, and `--check` compares the installed file against what the
*currently installed* citefinder would render. So the upgrade loop is:

```bash
uv add -U citefinder          # or: uv tool upgrade citefinder
citefinder install --check    # says `drifted` when the skill predates the CLI
citefinder install --force    # re-materialize at the new version
```

Add `--local` to both commands when the skill is vendored in a repo. Without
the stamp there is nothing tying a hand-copied skill to the version installed
next to it — the failure this replaces was a copy still documenting a pre-0.4
CLI shape in a repo whose lockfile pinned 0.4.2.

A file at the target path that citefinder did not generate (no stamp) is never
overwritten without `--force`, so a hand-authored skill of the same name is
safe. Edit the skill in `citefinder/prompts/skill.md` and re-run `install` —
never in a generated `.claude/` copy, which the next install replaces.
